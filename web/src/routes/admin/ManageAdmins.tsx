  ...
  return (
    <div className="card" style={{ width: '100%' }}>
      ...
      <ul style={{ listStyle: 'none', padding: 0 }}>
        {emails.map((e) => (
          <li key={e} style={{ display: 'flex', justifyContent: 'space-between', padding: '.3rem 0' }}>
            <span>{e}</span>
            <button onClick={() => runAdmin(removeAdmin(db, e))} style={{ background: 'none', border: 'none', color: '#a3322b', cursor: 'pointer' }}>remove</button>
          </li>
        ))}
      </ul>
      <div style={{ display: 'flex', gap: '.5rem' }}>
        ...
        <Button onClick={() => { if (email.trim()) { runAdmin(addAdmin(db, email.trim())); setEmail('') } }}>Add</Button>
      </div>
    </div>
  )
}
